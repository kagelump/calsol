...
def calc_dE(velocity, longitude, latitude, start_time=time.strftime("%H:%M", time.localtime()), end_time="17:00", cloudy=0):
        if velocity < 0:
        ...
        energy_change = powerGeneration(longitude, latitude, velocity, st, et, cloudy) - powerConsumption(longitude, latitude, velocity, time.mktime(et)-time.mktime(st))
        return energy_change

# function 2: Given energy, find velocity
def calc_V(energy, longitude, latitude, start_time = time.strftime("%H:%M", time.localtime()), end_time="17:00", cloudy=0):
        ...
        while current_iteration < iteration_limit:
                energy_change = powerGeneration(longitude, latitude, velocity_guess, st, et, cloudy) - powerConsumption(longitude, latitude, velocity_guess, time.mktime(et) - time.mktime(st))
                if math.fabs(energy_change - energy) < error:
                        return velocity_guess
                else: 
                        # Update velocity guess value
                        E_prime = (powerGeneration(longitude, latitude, velocity_guess+dv, st, et, cloudy) - powerConsumption(longitude, latitude, velocity_guess+dv, time.mktime(et)-time.mktime(st)) - energy_change) / dv
        ...
        while True:
                ...
                if operationType=="n":
                        ...
                        if calcType=="v":
                                ...
                                if cloudiness=="":
                                        couldiness=-1
                                print str(calc_V(float(inputEnergy),float(longitude),float(lat),startTime,endTime,float(cloudiness))) + "km/h"
                        ...
                        if calcType=="e":
                                ...
                                if cloudiness=="":
                                        couldiness=-1
                                print str(calc_dE(float(inputVelocity),float(longitude),float(lat),startTime,endTime,float(cloudiness))) + "MJ"
...
def powerGeneration(longitude, latitude, velocity, start_time, end_time, cloudy):
